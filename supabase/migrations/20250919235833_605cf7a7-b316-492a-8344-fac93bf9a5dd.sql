...
ALTER TABLE public.quote_requests DROP CONSTRAINT IF EXISTS check_email_format;
ALTER TABLE public.quote_requests 
ADD CONSTRAINT check_email_format 
CHECK (email ~* '^[A-Za-z0-9._+%-]+@[A-Za-z0-9.-]+[.][A-Za-z]+$');

ALTER TABLE public.quote_requests DROP CONSTRAINT IF EXISTS check_phone_format;
ALTER TABLE public.quote_requests 
ADD CONSTRAINT check_phone_format 
CHECK (phone IS NULL OR phone ~ '^[\d\s\(\)\-\+]+$');

ALTER TABLE public.quote_requests DROP CONSTRAINT IF EXISTS check_name_not_empty;
ALTER TABLE public.quote_requests 
