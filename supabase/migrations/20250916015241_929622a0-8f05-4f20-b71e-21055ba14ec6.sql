...
-- RLS Policies
CREATE POLICY "Logistics staff can manage packages"
...
CREATE POLICY "Logistics staff can manage rate cards"
...
CREATE POLICY "Logistics staff can manage postcode zones"
...
CREATE POLICY "Assembly team can manage assembly jobs"
...
CREATE POLICY "Logistics staff can manage depot list"
...
CREATE POLICY "Logistics staff can manage exceptions"
  ON public.exceptions FOR ALL
  USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'fulfilment'::app_role) OR has_role(auth.uid(), 'logistics'::app_role));

CREATE POLICY "Logistics staff can manage printing queue"
