-- SECURITY FIXES - Phase 3: Complete RLS policy implementation
...
-- Users can view payments for their own orders/invoices
...
USING (
  auth.uid() IS NOT NULL AND
  (
    -- Can view payments for their own invoices
    invoice_id IN (
      SELECT i.id 
      FROM invoices i
      JOIN orders o ON o.id = i.order_id
    ...
    OR
    -- Can view payments for their own checkouts
    checkout_id IN (
      SELECT c.id 
      FROM checkouts c
