...
const PanelsFillers = () => {
  ...
  const { data: cabinetTypes } = useQuery({
    queryKey: ['cabinet-types-panels'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('cabinet_types')
        .select('*')
        .eq('active', true)
        .in('category', ['panel', 'filler', 'accessory'])
        .order('name');
      ...
      for (const cabinetType of cabinetTypes) {
        ...
        if (typeFinishes.length > 0) {
          const ranges = typeRanges.length > 0 ? typeRanges : [
            { id: 'default-1', label: '50 - 100mm', min_width_mm: 50, max_width_mm: 100 },
            { id: 'default-2', label: '100 - 200mm', min_width_mm: 100, max_width_mm: 200 },
            { id: 'default-3', label: '200 - 300mm', min_width_mm: 200, max_width_mm: 300 }
          ];

          const hardwareCost = 15; // Lower hardware cost for panels/fillers
            ...
            sizes: ranges.map((range: any) => {
              ...
              const prices = typeFinishes.map((finish: any) => {
                if (!finish.door_style || !finish.door_style_finish) {
                  ...
                  return 50; // Default price for panels/fillers
                }
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6">
              Dress Panels & <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Fillers</span>
            </h1>
            <p className="text-xl text-muted-foreground mb-8 leading-relaxed">
              Finishing touches for your kitchen. Click any price to configure and add to your quote.
            </p>
            ...
            <div className="bg-muted/30 rounded-lg p-6 mb-8 max-w-2xl mx-auto">
              ...
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                <div className="text-left">
                  ...
                  <p className="text-muted-foreground">(matches cabinet height)</p>
                  <p className="text-primary font-medium mt-1">25% extra for custom height up to 1000mm</p>
                </div>
                <div className="text-left">
                  <p className="font-medium text-foreground">Standard Depth: 20mm</p>
                  <p className="text-muted-foreground">(panel thickness)</p>
                  <p className="text-primary font-medium mt-1">Custom depths available</p>
                </div>
              </div>
            </div>
          ...
          {(!cabinetTypes || cabinetTypes.length === 0) && (
            <div className="text-center py-12">
              <p className="text-muted-foreground">
                No panel/filler types configured yet. Please set up cabinet types in the admin panel.
              </p>
            </div>
          )}
        </div>
